          ...
          data.is_personal ? (
            ...
            <>
              <div className="flex flex-col sm:flex-row sm:items-center justify-between mb-6 gap-3">
                <div>
                  <h1 className="text-xl sm:text-2xl font-bold">Meus Recebimentos</h1>
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 mb-6">
                <div className="card p-4">
                  <p className="text-xs text-gray-500 mb-1">A Receber (contratos ativos)</p>
                  <p className="text-2xl font-bold text-blue-600">{fmt(data.total_receivable)}</p>
                  <p className="text-xs text-gray-400 mt-1">soma das suas alocações ativas</p>
                </div>
                <div className="card p-4">
                  <p className="text-xs text-gray-500 mb-1">Proporcional {MONTHS[month - 1]}</p>
                  <p className="text-2xl font-bold text-emerald-600">{fmt(data.total_operational_cost)}</p>
                  <p className="text-xs text-gray-400 mt-1">valor calculado no período</p>
                </div>
              </div>

              {/* Allocations by client */}
              ...
              {data.by_client.length === 0 ? (
                <div className="card p-8 text-center">
                  ...
                  <p className="text-gray-500">
                    {data.total_receivable === 0 && data.total_operational_cost === 0
                      ? 'Sua conta não está vinculada a um membro da equipe. Peça ao administrador para vincular.'
                ...
                <div className="space-y-3">
                  {data.by_client.map(c => (
                    <div key={c.client_id} className="card p-4">
                      ...
                      >
                        <div>
                          ...
                          <p className="text-xs text-gray-400">
                            {c.allocations[0]?.active_days}/{c.allocations[0]?.days_in_month} dias ativos
                          </p>
                        </div>
                        <div className="flex items-center gap-3">
                      ...
                      {expandedClient === c.client_id && c.allocations.map(a => (
                        <div key={a.allocation_id} className="mt-3 pt-3 border-t dark:border-dark-700 text-sm text-gray-500">
                          <div className="flex justify-between">
                            <span>Início: {new Date(a.start_date + 'T00:00:00').toLocaleDateString('pt-BR')}</span>
                            <span>{a.end_date ? `Fim: ${new Date(a.end_date + 'T00:00:00').toLocaleDateString('pt-BR')}` : 'Sem data de término'}</span>
                          </div>
                        </div>
                      ))}
                    </div>
